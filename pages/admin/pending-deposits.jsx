...
const PendingDeposit = () => {
  ...
  const fetchPending = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error(err.message || "Failed to fetch pending deposits");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleApproval = async (id) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/deposits/${id}/approve`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      console.error(error);
      toast.dismiss();
      toast.error(error || "Failed to approve deposit", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  const handleReject = async (id) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/deposits/${id}/decline`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      console.error(error);
      toast.dismiss();
      toast.error(error || "Failed to decline deposit", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
            ...
            <tbody>
              {pendingTransfers.length > 0 ? (
                ...
                <tr className="whitespace-nowrap">
                  <td>There are no Pending Deposits for now !</td>
                </tr>
              )}
            </tbody>
          </table>
